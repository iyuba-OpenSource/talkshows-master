package com.iyuba.talkshow.ui.lil.view.exoplayer;
 ...
 * 简单的视频控制器
 */
public class SimplePlayControlView extends FrameLayout {
    ...
    /**
     * @deprecated Register a {@link SimplePlayView.ControllerVisibilityListener} via {@link
     *     SimplePlayView#setControllerVisibilityListener(SimplePlayView.ControllerVisibilityListener)}
     *     instead. Using {@link SimplePlayControlView} as a standalone class without {@link
     *     SimplePlayView} is deprecated.
     */
    @Deprecated
    ...
    /**
     * @deprecated Register a {@link SimplePlayView.FullscreenButtonClickListener} via {@link
     *     SimplePlayView#setFullscreenButtonClickListener(SimplePlayView.FullscreenButtonClickListener)}
     *     instead. Using {@link SimplePlayControlView} as a standalone class without {@link
     *     SimplePlayView} is deprecated.
     */
    @Deprecated
    ...
    private final SimplePlayControlView.ComponentListener componentListener;

    @SuppressWarnings("deprecation") // Using the deprecated type for now.
    private final CopyOnWriteArrayList<SimplePlayControlView.VisibilityListener> visibilityListeners;

    @Nullable private final View previousButton;//上一个
    @Nullable private final View nextButton;//下一个
    @Nullable private final View playPauseButton;//播放
    @Nullable private final View fastForwardButton;//快进
    @Nullable private final View rewindButton;//快退
    @Nullable private final TextView fastForwardButtonTextView;//快进
    @Nullable private final TextView rewindButtonTextView;//后退
    @Nullable private final ImageView repeatToggleButton;//循环
    @Nullable private final ImageView shuffleButton;//随机
    @Nullable private final View vrButton;
    @Nullable private final TextView durationView;//总时长
    @Nullable private final TextView positionView;//当前时长
    @Nullable private final TimeBar timeBar;//进度条
    ...
    @Nullable private SimplePlayControlView.ProgressUpdateListener progressUpdateListener;

    @Nullable private SimplePlayControlView.OnFullScreenModeChangedListener onFullScreenModeChangedListener;
    ...
    private SimplePlayControlViewLayoutManager controlViewLayoutManager;
    private Resources resources;

    private RecyclerView settingsView;
    private SimplePlayControlView.SettingsAdapter settingsAdapter;
    private SimplePlayControlView.PlaybackSpeedAdapter playbackSpeedAdapter;
    private PopupWindow settingsWindow;
    private boolean needToHideBars;
    private int settingsWindowMargin;

    private SimplePlayControlView.TextTrackSelectionAdapter textTrackSelectionAdapter;
    private SimplePlayControlView.AudioTrackSelectionAdapter audioTrackSelectionAdapter;
    ...
    public SimplePlayControlView(Context context) {
        this(context, /* attrs= */ null);
    }

    public SimplePlayControlView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, /* defStyleAttr= */ 0);
    }

    public SimplePlayControlView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public SimplePlayControlView(
            Context context,
            @Nullable AttributeSet attrs,
            int defStyleAttr,
            @Nullable AttributeSet playbackAttrs) {
        super(context, attrs, defStyleAttr);
        int controllerLayoutId = R.layout.video_simple_control;
        ...
        componentListener = new SimplePlayControlView.ComponentListener();
        ...
        if (backView!=null){
            backView.setOnClickListener(v->{
                //回调返回方法
                EventBus.getDefault().post(new SimplePlayEvent(SimplePlayEvent.type_back));
            });
        }
        //播放
        playPauseButton = findViewById(R.id.playView);
        ...
        controlViewLayoutManager = new SimplePlayControlViewLayoutManager(this);
        ...
        settingsAdapter = new SimplePlayControlView.SettingsAdapter(settingTexts, settingIcons);
        ...
        textTrackSelectionAdapter = new SimplePlayControlView.TextTrackSelectionAdapter();
        audioTrackSelectionAdapter = new SimplePlayControlView.AudioTrackSelectionAdapter();
        playbackSpeedAdapter =
                new SimplePlayControlView.PlaybackSpeedAdapter(
    ...
    @Deprecated
    public void addVisibilityListener(SimplePlayControlView.VisibilityListener listener) {
    ...
    @SuppressWarnings("deprecation")
    @Deprecated
    public void removeVisibilityListener(SimplePlayControlView.VisibilityListener listener) {
        visibilityListeners.remove(listener);
    }

    /**
     * Sets the {@link SimplePlayControlView.ProgressUpdateListener}.
     *
     * @param listener The listener to be notified about when progress is updated.
     */
    public void setProgressUpdateListener(@Nullable SimplePlayControlView.ProgressUpdateListener listener) {
    ...
    public void setOnFullScreenModeChangedListener(
            @Nullable SimplePlayControlView.OnFullScreenModeChangedListener listener) {
    ...
    @SuppressWarnings("deprecation") // Calling the deprecated listener for now.
        ...
        for (SimplePlayControlView.VisibilityListener visibilityListener : visibilityListeners) {
    ...
    private void updatePlayPauseButton(boolean isClicked) {
        if (!isVisible() || !isAttachedToWindow) {
            return;
        }
        if (playPauseButton != null) {
            if (shouldShowPauseButton()) {
//                ((ImageView) playPauseButton).setImageDrawable(resources.getDrawable(R.drawable.exo_styled_controls_pause));
                ((ImageView) playPauseButton).setImageDrawable(resources.getDrawable(R.drawable.pause));
                playPauseButton.setContentDescription(resources.getString(R.string.exo_controls_pause_description));
            } else {
//                ((ImageView) playPauseButton).setImageDrawable(resources.getDrawable(R.drawable.exo_styled_controls_play));
                ((ImageView) playPauseButton).setImageDrawable(resources.getDrawable(R.drawable.play));
                playPauseButton.setContentDescription(resources.getString(R.string.exo_controls_play_description));
            }
            ...
            if (isClicked){
                if (shouldShowPauseButton()){
                    //开始播放
                    EventBus.getDefault().post(new SimplePlayEvent(SimplePlayEvent.type_play));
                }else {
                    //开始暂停
                    EventBus.getDefault().post(new SimplePlayEvent(SimplePlayEvent.type_pause));
                }
            }
        }
    }
    ...
    private ImmutableList<SimplePlayControlView.TrackInformation> gatherSupportedTrackInfosOfType(
            Tracks tracks, @C.TrackType int trackType) {
        ImmutableList.Builder<SimplePlayControlView.TrackInformation> trackInfos = new ImmutableList.Builder<>();
        ...
        for (int trackGroupIndex = 0; trackGroupIndex < trackGroups.size(); trackGroupIndex++) {
            ...
            for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
                ...
                trackInfos.add(new SimplePlayControlView.TrackInformation(tracks, trackGroupIndex, trackIndex, trackName));
            }
        }
    ...
    private final class ComponentListener
        ...
        public void onClick(View view) {
            @Nullable Player player = SimplePlayControlView.this.player;
            if (player == null) {
                return;
            }
            controlViewLayoutManager.resetHideCallbacks();
    ...
    private class SettingsAdapter extends RecyclerView.Adapter<SimplePlayControlView.SettingViewHolder> {
        ...
        @Override
        public SimplePlayControlView.SettingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View v =
                    LayoutInflater.from(getContext())
                            .inflate(R.layout.exo_styled_settings_list_item, parent, /* attachToRoot= */ false);
            return new SimplePlayControlView.SettingViewHolder(v);
        }

        @Override
        public void onBindViewHolder(SimplePlayControlView.SettingViewHolder holder, int position) {
    ...
    private final class PlaybackSpeedAdapter extends RecyclerView.Adapter<SimplePlayControlView.SubSettingViewHolder> {
        ...
        @Override
        public SimplePlayControlView.SubSettingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View v =
                    LayoutInflater.from(getContext())
                            .inflate(
                                    R.layout.exo_styled_sub_settings_list_item, parent, /* attachToRoot= */ false);
            return new SimplePlayControlView.SubSettingViewHolder(v);
        }

        @Override
        public void onBindViewHolder(SimplePlayControlView.SubSettingViewHolder holder, int position) {
    ...
    private final class TextTrackSelectionAdapter extends SimplePlayControlView.TrackSelectionAdapter {
        @Override
        public void init(List<SimplePlayControlView.TrackInformation> trackInformations) {
        ...
        @Override
        public void onBindViewHolderAtZeroPosition(SimplePlayControlView.SubSettingViewHolder holder) {
        ...
        public void onBindViewHolder(SimplePlayControlView.SubSettingViewHolder holder, int position) {
            ...
            if (position > 0) {
                SimplePlayControlView.TrackInformation track = tracks.get(position - 1);
    ...
    private final class AudioTrackSelectionAdapter extends SimplePlayControlView.TrackSelectionAdapter {
        ...
        public void init(List<SimplePlayControlView.TrackInformation> trackInformations) {
            ...
            if (trackInformations.isEmpty()) {
                ...
                for (int i = 0; i < trackInformations.size(); i++) {
                    SimplePlayControlView.TrackInformation track = trackInformations.get(i);
                    if (track.isSelected()) {
                        settingsAdapter.setSubTextAtPosition(
    ...
    private abstract class TrackSelectionAdapter extends RecyclerView.Adapter<SimplePlayControlView.SubSettingViewHolder> {

        protected List<SimplePlayControlView.TrackInformation> tracks;

        protected TrackSelectionAdapter() {
            this.tracks = new ArrayList<>();
        }

        public abstract void init(List<SimplePlayControlView.TrackInformation> trackInformations);
        ...
        protected abstract void onBindViewHolderAtZeroPosition(SimplePlayControlView.SubSettingViewHolder holder);
