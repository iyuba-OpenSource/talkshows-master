...
public class WavWriter {
    ...
    public WavWriter(File file, int sample) throws IOException {
        this(file, DEFAULT_NUM_CHANNELS, sample, DEFAULT_BITS_PER_SAMPLE);
    }

    public WavWriter(File file, short numChannels, int sample, short bitsPerSample) throws
            IOException {
        init(file, numChannels, sample, bitsPerSample);
    }

    private boolean init(File file, short numChannels, int sampleRate, short bitsPerSample)
            throws IOException {
        if (null == file)
            return false;
