...
public class VestBagAdapter extends RecyclerView.Adapter<VestBagAdapter.VestBagHolder> {
    ...
    @NonNull
    @Override
    public VestBagAdapter.VestBagHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ItemCourseDetailBinding binding = ItemCourseDetailBinding.inflate(LayoutInflater.from(context),parent,false);
        return new VestBagAdapter.VestBagHolder(binding);
    }

    @Override
    public void onBindViewHolder(@NonNull VestBagAdapter.VestBagHolder holder, int position) {
        if (holder==null){
            return;
        }
