    ...
    public void play(final Device device, final String path) {
        ...
        executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                Log.d("diaodiao1", System.currentTimeMillis()+"");

                if (playerMonitor != null) {
                    HANDLER.post(new Runnable() {
                        @Override
                        public void run() {
                            Log.d("diaodiao2", System.currentTimeMillis()+"");

                            isPlaying = true;
//                            ret = true ;
                            if (true) {
                                playerMonitor.onPlay();
                                Log.d("diaodiao3", System.currentTimeMillis()+"");

                                getMediaDuration(device);
                                Log.d("diaodiao4", System.currentTimeMillis()+"");

                                getMaxVolumeValue(device);
                                Log.d("diaodiao5", System.currentTimeMillis()+"");

                                getVolume(device);
                                Log.d("diaodiao6", System.currentTimeMillis()+"");

                            } else {
                                playerMonitor.onError();
                            }
                            Log.d("diaodiao7", System.currentTimeMillis()+"");
...
//                                HANDLER.removeMessages(MSG_UPDATE_PROGRESS);
    ...
    private synchronized boolean playSync(Device device, String path) {
        Log.d("diaodiao", System.currentTimeMillis()+"");
    ...
    private synchronized String getPositionInfoSync(Device device) {
        ...
        localAction.setArgumentValue("InstanceID", "0");
        for (int i = 0; i < localAction.getArgumentList().size(); i++) {
            Log.d("com.iyuba.talkshow",localAction.getArgumentList().getArgument(i).getArgumentNode().getName());
        }
    ...
    private synchronized String getMediaDurationSync(Device device) {
        ...
        if (localAction.postControlAction()) {
            Log.d("com.iyuba.talkshow", localAction.getArgumentValue("CurrentTransportState"));
            Log.d("com.iyuba.talkshow", localAction.getArgumentValue("MediaDuration"));
