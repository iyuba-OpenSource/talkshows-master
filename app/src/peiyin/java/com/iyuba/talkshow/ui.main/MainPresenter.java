...
public class MainPresenter extends BasePresenter<MainMvpView> {
    ...
    public void getVoas() {
        checkViewAttached();
        RxUtil.unsubscribe(mLoadSub);
        mLoadSub = mDataManager.getHomeVoas()
...
//                            getMvpView().showToast(R.string.request_more_data);
    ...
    public void loadMoreNewVoas(int pageNum, final CategoryFooter category) {
        mDataManager.getHomeNewList(pageNum)
                ...
                .subscribe(new Subscriber<List<Voa>>() {
                    ...
                    public void onNext(List<Voa> voas) {
                        if (voas != null && voas.size() < 4) {
                            ...
                            getMvpView().showMoewNewList(voas, category);
                        }
                    }
                });
    }
...
//                            loadNewVoas(id);
    ...
    protected void getVoa4Category(String ids) {
        Log.e("getVoa4Category", "执行了！");
        RxUtil.unsubscribe(mGetVoa4Ids);
        mGetVoa4Ids = mDataManager.getVoa4Category(ids, UserInfoManager.getInstance().getUserId())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<List<Voa>>() {
                    @Override
                    public void onCompleted() {
...
//                            getVoas();
                            Log.e("getVoa4Category", " onNext  response.size() " + voas.size());
                        } else {
...
//                : LoopService.GetLoopInfo.Param.Value.LOOP_TYPE;
        mLoadLoopSub = mDataManager.getLunboItems("class.childtalkshow")
                .compose(RxUtil.io2main())
...
//                            getMvpView().showToast(R.string.request_fail);
    ...
    public void getSoundByVoaId(int voaId ,int paraid , int index ){
        checkViewAttached();
        RxUtil.unsubscribe(mGetVoaByIdSub);
        mGetVoaByIdSub = mDataManager.getVoaById(voaId)
                .compose(RxUtil.io2main())
                .map(new Func1<Voa, String>() {
                    @Override
                    public String call(Voa voa) {
                        String sound = voa.sound().replace(".mp3","");
                        return Constant.Web.SOUND_PREFIX+sound+"/"+voa.voaId()+"_"+paraid+"_"+index+".wav";
                    }
                })
                .subscribe(new Subscriber<String>() {
                    ...
                    @Override
                    public void onNext(String voa) {
                        if (getMvpView() != null) {
                            EventBus.getDefault().post(new GetSoundEvent(voa,voaId,paraid,index));
                        }else {
                            getMvpView().showToast(R.string.database_error);
                        }
                    }
...
//        BrandUtil.requestQQGroupNumber(mDataManager.getPreferencesHelper(), mAccountManager.getUid(), App.APP_ID);
    ...
    public void getAllWords() {
        mDataManager.getAllWords().compose(RxUtil.io2main())
                .subscribe(new Subscriber<AllWordsRespons>() {
                    ...
                    public void onError(Throwable e) {
                        e.printStackTrace();
                        getMvpView().showToast("单词获取失败");
                    }

                    @Override
                    public void onNext(AllWordsRespons allWordsRespons) {
                        getMvpView().onWordsReady(allWordsRespons);
                    }
                });
    }

    public void getBookWords(int bookId) {
        mDataManager.getWordsByBookId(String.valueOf(bookId)).compose(RxUtil.io2main())
                .subscribe(new Subscriber<AllWordsRespons>() {
                    @Override
                    public void onCompleted() {
                    }
    ...
    public void refreshWords(int bookId, int version) {
        mDataManager.updateWordByBookId(bookId, version)
                .compose(RxUtil.io2main())
                .subscribe(new Subscriber<UpdateWordResponse>() {
                    ...
                    public void onNext(UpdateWordResponse response) {
                        if (response.getResult() == 200) {
                            int v = 1;
                            for (TalkShowWords word : response.getData()) {
                                v += word.version;
                            }
                            getMvpView().refreshBookWord(bookId, v, response.getData());
                        } else {
                            ToastUtils.showShort("数据已是最新");
                        }
                    }
                });
    }
