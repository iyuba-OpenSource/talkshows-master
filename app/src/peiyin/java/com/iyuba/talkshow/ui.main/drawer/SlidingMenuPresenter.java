...
public class SlidingMenuPresenter extends BasePresenter<SlidingMenuMvpView> {
    ...
    public void refreshBookWords() {
        int bookId = mConfigManager.getCourseId();
        int version = 0 ;
        BookLevels levels = WordDataBase.getInstance(((Fragment)getMvpView()).getActivity()).getBookLevelDao().getBookLevel(bookId);
        if (levels != null ){
