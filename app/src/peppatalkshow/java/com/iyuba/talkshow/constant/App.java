package com.iyuba.talkshow.constant;

import com.iyuba.lib_common.data.Constant;
import com.iyuba.talkshow.ui.courses.courseChoose.TypeHelper;

/**
 * App
 */
public interface App {
    int APP_ID = 280;
    String LOOP_TYPE = "class.peppapig";
    String APP_NAME_EN = "peppatalkshow";
    String APP_NAME_CH = "小猪英语";
    String APP_NAME_WEIXIN = "小猪英语";
    String APP_NAME_PRIVACY = "隐私政策";
    String PLATFORM = "android";
    boolean APP_CHECK_UPGRADE = true;
    boolean APP_TENCENT_PRIVACY = true;
    boolean APP_HUAWEI_PRIVACY = true;
    boolean APP_HUAWEI_COMPLAIN = false;
    boolean APP_MINI_PRIVACY = false;
    boolean APP_TENCENT_MOOC = true;
    boolean APP_CHECK_PERMISSION = false;
    boolean APP_CHECK_AGREE = false;
    boolean APP_CHECK_VIVO = false;
    int APP_SHARE_WXMINIPROGRAM = 1;
    int APP_SHARE_PART = 1;
    int APP_SHARE_HIDE = 0;

    String SMS_APP_KEY = "351d58f090f34";
    String SMS_APP_SECRET = "85c0e79a3ccaaabd74472607c66d9b80";
    String APP_SAVE_DIR = "appUpdate";
    String UNDERLINE = "_";
    String APK_SUFFIX = ".apk";
    String AD_FOLDER = "ad";
    String SHARE_NAME_EN = "peppatalkshow";
    String UMENG_KEY = "5ea2ad48570df336590000e2";

    String ALIPAY_APP_ID = "2016050601368497";
    String RSA_PRIVATE = "";

    String DEFAULT_WORDS = "middle";
    String DEFAULT_TYPE = "primary";

    int COURSE_TYPE = TypeHelper.DEFAULT_TYPE;
    class Url {
        public static String APP_ICON_URL = "https://www.ibbc.net.cn/app_pig.png";
        //山东爱语吧-其他
        public static String USAGE_URL = Constant.Url.PROTOCOL_SDIYB_USAGE;
        public static String PROTOCOL_URL = Constant.Url.PROTOCOL_SDIYB_PRIVACY;
        public static String CHILD_PROTOCOL_URL = Constant.Url.CHILD_PROTOCOL_SDIYB_PRIVACY;
        public static String VIP_AGREEMENT_URL = Constant.Url.VIP_AGREEMENT_SDIYB_PRIVACY;

        public static String SHARE_APP_URL = "http://app.iyuba.cn/";
    }

    //是否判断为儿童类型
    class Apk {
        public static boolean isChild() {
            return App.APP_ID == 256;
        }
    }

    /****oaid升级****/
    //证书需要每年更换-2023年01月12日
    //oaid的证书名称
    String oaid_pem = "com.iyuba.talkshow.pappa.cert.pem";
    //是否升级oaid
    boolean OAID_UPDATE = true;


    //课程相关数据
    //默认的数据
    int DEFAULT_BOOKID = 205;
    String DEFAULT_TITLE = "1年级上(新起点)";

    String DEFAULT_SERIESID = "309";

    //暂无用途
    boolean OPEN_WORD = true;//单词界面界面展示开关
    String DEFAULT_SERIES_TITLE = "新起点";
    String DEFAULT_SOURCE = "人教版";
    String DEFAULT_LESSON_TYPE = "primary";//junior-初中，primary-小学
}
