...
//                            getSyncVoas();
    ...
    public void syncSeries(String cat) {
        checkViewAttached();
        RxUtil.unsubscribe(mLoadSeriesSub);
        mLoadSeriesSub = mDataManager.syncSeries4Category(cat, UserInfoManager.getInstance().getUserId())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
...
//                getMvpView().showToast("失败");
                    ...
                    public void onNext(UidBean uidBean) {
                        if (getMvpView()!=null){
                            if (uidBean.getResult() == 200){
                                getUserInfo(uidBean.getUid(),true);
                            }else {
                                getMvpView().showWxLoginMsg("未获取到用户信息");
                            }
                        }
    ...
    //获取用户的信息
    public void getUserInfo(int uid,boolean isShowWxResult){
        /*checkViewAttached();
        RxUtil.unsubscribe(mUserInfoSub);
        mUserInfoSub = mDataManager.getUserInfo(uid,uid)
                ...
                .subscribe(new Subscriber<UserData>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        if (getMvpView()!=null&&isShowWxResult){
